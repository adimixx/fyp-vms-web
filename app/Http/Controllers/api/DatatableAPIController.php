...
class DatatableAPIController extends Controller
{
    ...
    private function complaint(Request $request, bool $isPending = false)
    {
        ...
        if ($isPending) {
            $data = $data->where('complaints.status', '1');
        } else {
            $data = $data->where('complaints.status', '!=', '1');
        }
    ...
    {
        $data = MaintenanceRequest::select('maintenance_requests.*', 'inv.reg_no AS reg_no',  DB::raw('CONCAT_WS(\' \',ctlg.brand, ctlg.model, ctlg.variant, CASE WHEN ctlg.year IS NOT NULL THEN CONCAT(\'(\', ctlg.year, \')\') ELSE NULL END) AS model'))
        ...
        if ($isPending) {
            $data = $data->where('maintenance_requests.status', '1');
        } else {
            $data = $data->where('maintenance_requests.status', '!=', '1');
        }
